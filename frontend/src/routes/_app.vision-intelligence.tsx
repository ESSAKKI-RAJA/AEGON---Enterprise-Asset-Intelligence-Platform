...
import {
  ScanEye, RefreshCw, Cpu, Activity, ShieldAlert, BarChart2, Clock, DollarSign,
  AlertTriangle, CheckCircle2, Play, RotateCcw, Globe, Search, Database, Layers,
  Server, Wifi, Wrench, Zap
} from "lucide-react";
...
import { 
  useUploadView, useGenerate360, useGenerateReport, 
  useInspectionHistory, useVisionStatistics, useCreateMaintenanceTicket 
} from "@/services/visionService";
import { assetService } from "@/services/assets.service";
import type {
  ViewPanelState, VisionViewType, DigitalTwinState, CompositeAnalysis,
  ExecutiveSummary, DefectFinding, InspectionHistoryItem
} from "@/types/vision";
...
function AnimatedKpi({ label, value, unit = "", icon: Icon, color = "text-slate-200", subtext }: any) {
  return (
...
function AssetSelector({ onSelect }: { onSelect: (asset: any) => void }) {
  ...
  return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center p-8">
      ...
      <h2 className="text-2xl font-mono font-bold text-slate-200 mb-2">Select Asset for Inspection</h2>
      <p className="text-slate-500 font-mono text-sm mb-8 text-center max-w-md">
        Connect Vision Intelligence to a registered asset to enable Digital Twin synchronization and automated maintenance workflows.
      </p>
      ...
      <div className="w-full max-w-xl bg-slate-900/80 border border-slate-700 p-2 rounded-xl">
        ...
        <div className="space-y-1 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
          {isLoading ? (
            <div className="p-4 text-center text-slate-500 font-mono text-xs animate-pulse">Loading registry...</div>
          ) : data?.items?.length === 0 ? (
            ...
            data?.items?.map((asset: any) => (
              ...
              >
                <div>
                  <div className="text-sm font-bold text-slate-300 group-hover:text-indigo-400 font-mono">{asset.name}</div>
                  <div className="text-[10px] text-slate-500 font-mono mt-1">SN: {asset.serial_number || 'N/A'} • {asset.category}</div>
                </div>
                <div className="text-right">
                   <div className="text-[10px] font-mono text-emerald-400 bg-emerald-400/10 px-2 py-0.5 rounded">Ready</div>
...
function VisionIntelligencePage() {
  ...
  const [panels, setPanels] = useState<Record<VisionViewType, ViewPanelState>>(() => 
    Object.fromEntries(VIEW_PANELS.map(({ key, label }) => [key, { viewType: key, label, status: "idle" }])) as any
  );
  const [digitalTwin, setDigitalTwin] = useState<DigitalTwinState>({
    asset_name: "Asset", health_score: 100, risk_score: 0, temperature_celsius: 25,
    pressure_psi: 14.7, rotation_rpm: 0, maintenance_status: "Up to Date", inspection_progress_pct: 0,
    views_completed: [], historical_health_trend: [], historical_risk_trend: [], last_updated: new Date().toISOString(),
  });
  ...
  const [activeTab, setActiveTab] = useState<"findings" | "analytics" | "summary" | "history">("findings");
  ...
  const uploadMutation = useUploadView((data) => {
    ...
    setPanels((prev) => ({ ...prev, [vt]: { ...prev[vt], status: "complete", result: data.view_result } }));
    if (data.digital_twin) setDigitalTwin(data.digital_twin);
  });

  const generate360Mutation = useGenerate360((data) => {
    setComposite(data.composite); setExecSummary(data.executive_summary);
    if (data.digital_twin) setDigitalTwin(data.digital_twin);
  ...
  const ticketMutation = useCreateMaintenanceTicket((data) => {
    toast.success(`Maintenance Ticket ${data.ticket_id} created successfully!`, { icon: <Wrench className="h-4 w-4 text-emerald-400"/> });
  });

  if (!selectedAsset) {
    return <PageLayout><AssetSelector onSelect={setSelectedAsset} /></PageLayout>;
  }

  const handleFileSelectAndAnalyze = (viewType: VisionViewType, file: File) => {
    setPanels(p => ({ ...p, [viewType]: { ...p[viewType], status: "analyzing", previewUrl: URL.createObjectURL(file) } }));
    uploadMutation.mutate({ viewType, file, sessionId: sessionId.current, assetName: selectedAsset.name, operator: "Admin" });
  };

  const handleAnalyze = (viewType: VisionViewType) => {
    setPanels(p => ({ ...p, [viewType]: { ...p[viewType], status: "analyzing" } }));
    uploadMutation.mutate({ viewType, file: undefined, sessionId: sessionId.current, assetName: selectedAsset.name, operator: "Admin" });
  };
  ...
  const handleRunAllMock = () => {
    VIEW_PANELS.forEach(({ key }, i) => {
      setTimeout(() => {
        ...
        uploadMutation.mutate({ viewType: key, sessionId: sessionId.current, assetName: selectedAsset.name, operator: "Admin" });
      }, i * 800);
    });
  };

  const allFindings = Object.values(panels).filter(p => p.result).flatMap(p => p.result!.findings);
  ...
  const lastResult = Object.values(panels).reverse().find(p => p.result)?.result;
  ...
  return (
    <PageLayout>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between bg-slate-900 border border-slate-800 p-3 rounded-lg mb-6 gap-4">
        ...
        <div className="flex items-center gap-4">
           <div className="flex items-center gap-2">
             ...
             <div className="text-[10px] font-mono font-bold text-emerald-400 uppercase">Connected</div>
           </div>
           <div className="text-xs font-mono text-slate-400 border border-slate-700 px-2 py-1 rounded bg-slate-950">{currentTime}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
        <AnimatedKpi label="Health Score" value={composite?.overall_health_score ?? digitalTwin.health_score} unit="%" icon={Activity} color="text-teal-400" />
        <AnimatedKpi label="Risk Score" value={composite?.risk_score ?? digitalTwin.risk_score} unit="%" icon={ShieldAlert} color="text-amber-400" />
        <AnimatedKpi label="Latency" value={latency.toFixed(0)} unit="ms" icon={Zap} color="text-indigo-400" />
        <AnimatedKpi label="Queue" value={lastResult?.queue_position || 0} icon={Layers} color="text-blue-400" />
        <AnimatedKpi label="Total Defects" value={allFindings.length} icon={AlertTriangle} color="text-amber-400" />
        <AnimatedKpi label="Critical" value={allFindings.filter(f=>f.severity==="critical").length} icon={AlertTriangle} color="text-red-400" />
        <AnimatedKpi label="Est. Cost" value={composite ? `$${(composite.maintenance_cost_estimate_usd/1000).toFixed(0)}K` : "—"} icon={DollarSign} color="text-teal-400" />
        <AnimatedKpi label="RUL" value={composite ? composite.remaining_useful_life_years : "—"} unit={composite?"yrs":""} icon={Clock} color="text-purple-400" />
      </div>

      <div className="grid gap-6 xl:grid-cols-[1fr_300px] mt-6">
        {/* SECTION 6: Multi View Inspection */}
        <div className="space-y-4">
          <SectionHeader title="Multi-View Inspection Matrix" icon={<ScanEye className="h-4 w-4 text-teal-400" />} />
            ...
            <div className="space-y-1">
               <div className="relative rounded border border-indigo-500/30 bg-slate-900 overflow-hidden flex flex-col min-h-[200px]">
                 <div className="p-2 border-b border-slate-800 bg-slate-950/50 flex justify-between items-center">
                    <span className="text-[10px] font-mono font-bold text-slate-300">360° SYNTHESIS</span>
                 </div>
                 <div className="flex-1 flex flex-col items-center justify-center p-4">
                    {!composite ? (
                      <div className="text-center">
                         <Globe className={cn("h-12 w-12 mx-auto mb-2", canGenerate360 ? "text-indigo-400 animate-pulse" : "text-slate-700")} />
                         <button
                      ...
                      <div className="text-center w-full">
                         <div className="text-3xl font-mono text-emerald-400 font-bold">{composite.overall_health_score.toFixed(0)}</div>
                         <div className="text-[9px] font-mono text-slate-500">FINAL HEALTH</div>
                         <div className="text-xs font-mono text-amber-400 mt-2">Risk: {composite.risk_score.toFixed(0)}%</div>
                      </div>
                    )}
                 </div>
               </div>
        ...
        <div>
          <SectionHeader title="Live Digital Twin" icon={<Database className="h-4 w-4 text-amber-400" />} />
          ...
          {[
            ...
            <button
              ...
              className={cn(
                ...
                activeTab === id ? "border-teal-400 text-teal-400 bg-teal-950/10" : "border-transparent text-slate-500 hover:text-slate-300"
              )}
            >
              {label}
            </button>
        ...
        <div className="p-5 min-h-[300px]">
           ...
           {activeTab === "findings" && (
              <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
                {Object.entries(panels).filter(([,p]) => p.result?.findings.length).map(([k, p]) => (
                  <div key={k}>
                    <p className="text-[10px] font-mono font-bold text-slate-500 uppercase mb-2">{p.label}</p>
                    <FindingsPanel 
                       findings={p.result!.findings} 
                       onCreateTicket={(f) => ticketMutation.mutate({ 
           ...
           {activeTab === "analytics" && <SeverityAnalytics findings={allFindings} digitalTwin={digitalTwin} />}
           {/* SECTION 12: Executive Summary */}
           {activeTab === "summary" && execSummary && composite && (
              <ExecutiveSummaryPanel summary={execSummary} composite={composite} onGenerateReport={() => reportMutation.mutate({sessionId: sessionId.current})} isGeneratingReport={reportMutation.isPending} />
           )}
           {activeTab === "summary" && (!execSummary || !composite) && (
              <div className="py-12 text-center text-slate-500 font-mono text-sm">Generate 360° Composite to view Executive Summary.</div>
           )}
